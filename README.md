...
Wariant B został sprawdzony na maszynie goszczącej pod Windows 11 z maszynami wirtualnymi Ubuntu i Debian pod VirtualBox. Ze względu na stwierdzone mankamenty, zwłaszcza dużą niestabilność pomiarów wynikającą z niskiej wydajności maszyn wirtualnych, użycie wariantu B należy uznać za absolutną **ostateczność**. Naszym zdaniem może on być użyty w przypadku, gdy zespół ma dostęp tylko do maszyn z systemem MacOS. W MacOS istnieją możliwości konfigurowania urządzeń sieciowych funkcjonalnie równoważnych używanym przez nas urządzeniom linuksowym, jednak nasz linuksowy skrypt w tym środowisku nie działa. Natomiast w ramach przygotowywania niniejszego ćwiczenia prac nad przeniesieniem wariantu B także na MacOS nie podjęto.
