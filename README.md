...
Ćwiczenie obejmie serie badań sieci, każda dotycząca innej charakterystyki ruchu pakietowego, a ich wyniki zostaną wzajemnie porównane. Podstawowym krokiem (składnikiem) każdej serii badań będzie eksperyment elementarny (próba), a seria będzie obejmować pewną liczbę prób, których wyniki zostaną uśrednione. W ramach strojenia środowiska może być wymagane przeprowadzenie prób wstępnych (testowych) - umożliwią one dobór dobrego punktu pracy systemu (pracujemy zwykle w środowisku zwirtualizowanym, na zróżnicowanym sprzęcie, i w każdym takim przypadku może być wskazany indywidualny dobór wartości głównych parametrów naszego "systemu"). Eksperymet elementarny jest opisany w sekcji xxxxxxxxxxxx, a zadania do wykonania (w tym wymagane serie badań sieci) określono w sekcji YYYYYYY.
