...
* Teraz, dla każdego punktu pomiarowego odniesienia, przeprowadzamy właściwą _punktową serię pomiarową_:
  ...
  * po wykonaniu wszystkich pomiarów elementarnych w ramach punktowej serii pomiarowej uśredniamy wartość każdej z wymienionych metryk. Przy sporządzaniu wykresów, średniówki opóźnienia i strat pakietów powinny znaleźć się w punkcie o współrzędnej na osi odciętej równej wartości średniej wyrażenia $` (\mbox{Total\_packets} + \mbox{Packets\_dropped}) / 1000 `$ z danej punbktowej serii pomiarowej (chociaż punkt odniesienia wskazuje na teoretyczną, różną od powyższej, intensywność napływu pakietów równą `X`).
