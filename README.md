...
Prosty demonstrator sieci z ruchem samopodobnym. Repozytorium zawiera opis ćwiczenia laboratoryjnego, podczas którego demonstrujemy wpływ wariancji strumienia pakietów kierowanych na wyjściowy imnterfejs urządzenia przełączającego na opóźnienie i straty pakietów.
...
> [!Note]
> W zamyśle ćwiczenie ma zilustrować **istotę** wpływu charakterystyki ruchu pakietowego (płynny, losowy, wybuchowy/samopodobny) na metryki transferu pakietów (strata, opóźnienie, etc.). Z założenia powinno być też niskonakładowe - realizowane z użyciem sprzętu powszechnego użytku. Dlatego konfiguracja środowiska sieciowego (w szczególności wielkość bufora w obserwowanym interfejsie przełącznika sieciowego) znacznie odbiega od tego, co moglibyśmy zobaczyć w dużej sieci. Ważne jest, że nawet przy takich uproszczeniach główny cel ćwiczenia nadal z powodzeniem daje się osiągnąć.

Środowisko laboratoryjne oparte jest na maszynach fizycznych lub wirtualnych pracujących pod systemem Linuks. Pracujemy z prostą siecią emulowaną przez parę sieciowych przestrzeni nazw (ang. _newtork namespace_), reprezentujących terminale końcowe (hosty), które są dołączone do przełącznika realizowanego przez wirtualne urządzenie linux bridge, reprezentującego ruter (urządzenie komutacji pakietów) przenoszący ruch pakietowy pomiędzy hostami. Do generowania ruchu wykorzystujemy aplikację D-ITG (manual jest dostępny [tutaj](https://traffic.comics.unina.it/software/ITG/manual/)). Schemat naszej sieci przedstawiono na poniższym rysunku. Bloki oznaczone jako hi-s1 oraz s1-hi to interfejsy należące do linuksowych urządzeń typu _veth pair_ (ang. `virtual eth pair`). Pary te reprezentują "kable" ethernetowe łączące poszczególne urządzenia (więcej o parach veth, a także o `linux bridge`, znajdziemy w dokumentacji Linuksa oraz w innych licznych źródłach dostępnych w Internecie). W naszym przypadku strona nadawcza D-ITG (moduł `ITGSend`) działa w hoście `h1`, a w hoście `h2` działa strona odbiorcza D-ITG (moduł `ITGRecv`).
...
W ramach ćwiczenia porównujemy wartości wybranych metryk jakościowych transferu pakietów w relacji `h1`-`h1` (por. wcześniejszy rysunek) dla różnych charakterystyk zmienności strumienia ruchu pakietowego w tej relacji. Ważniejsze metryki jakościowe transferu to strata pakietów, opóźnienie, _jitter_. Podstawowe typy zmienności strumieni ruchu, które wykorzystamy, to przepływność pakietowa stała (ang. `constant packet rate`), napływ pakietów poissonowski oraz ruch typu _ON/OFF_ z założoną charakterystyką ruchu w okresach aktywności _ON_ (np. ruch typu `constant packet rate` lub poissonowski w okresie _ON_).

Z wykorzystaniem generatora ruchu D-ITG można generować ruch także o innych własnościach niż powyżej wspomniane, np. ruch z czasem pomiędzy kolejnymi pakietami lub czas trwania stanów ON/OFF dla źródeł ON/OFF opisane rozkładem Weibull'a (ruch samopodobny, całkiem dobrze modelujący ruch w rzeczywistych sieciach IP, zwłaszcza w płaszczyźnie "core", patrz np. [tutaj](https://www.sciencedirect.com/science/article/abs/pii/S1084804519301547)). Za realizację dodatkowego testu, przy założeniu rozkładu Weibull'a dla czasu między kolejnymi pakietami, **zespołowi będzie przysługiwać bonus w wysokości 20%** nmominalnego _maksa_ za ćwiczenie.

> [!Note]
> Ze względu na uwarunkowania środowiska laboratoryjnego oraz rozwiązania implementacyjne generatora D-ITG poszczególne przebiegi pomiarowe (`eksperymenty elementarne`) przeprowadzane dla tego samego zbioru wartości parametrów opisujących zmienność generowanego strumienia ruchu dają różne wyniki. Dla danego zbioru parametrów wymagane jest więc uśrednienie wyników zebranych z szeregu przebiegów (eksperymentów elementarnych); nasze doświadczenia wskazują, że 10 prób pozwala uzyskać zadowalającą dokładność średniówki.

# Eksperyment elementarny
...
Skrypt ten zawiera szereg komentarzy wyjaśniających istotne dla nas kwestie szczegółowe. Komentarze te są zamieszczone bezpośrednio w kontekście odpowiednich komend zawierających stosowne parametry, a więc są możliwie dobrze skorelowane z ich przedmiotem. Dlatego w niniejszym dokumencie zadowalamy się opisem ogólnym, po techniczne detale odsyłając do samego skryptu.

Po wywołaniu skryptu komendą `sudo ./lbr.sh` tworzona jest sieć o topologii zilustrowanej wcześniej, a w hoście `h2` (w przestrzeni nazw sieciowych `h2`) uruchamiany jest odbiornik aplikacji D-ITG o nazwie `ITGRecv`. Odbiornik ten, na wybranym porcie (i na wszystkich interfejsach maszyny), nasłuchuje nadchodzących pakietów generowanych przez stronę nadawczą, `ITGSend`. W ramach tego skryptu NIE jest jednak uruchamiany nadajnik `ITGSend`. Skrypt `lbr.sh` wykonujemy tylko raz, na samym początku ćwiczenia.
...
Po wykonaniu skryptu `lbr.sh` można przystąpić do realizacji serii eksperymentów. Jeden eksperyment polega na wygenerowaniu strumienia pakietów pomiędzy `ITGSend` oraz `ITGRecv` i zarejestrowaniu jego wyników dostępnych w formie pewnych statystyk podanych w logu. W tym celu należy:
  * W **odrębnym oknie terminala** uruchomić stronę nadawczą, `ITGSend`, skonfigurowaną z odpowiednimi parametrami podawanymi w linii komendy (m.inn. adres strony odbiorczej, charakterystyka ruchu, czas trwania symulacji, nazwy logów nadawczego i odbiorczego). Moduł `ITGSend` uruchamiamy w przestrzeni nazw `h1`, a przykładowa komenda ma formę (o detalach parametryzacji jest dalej - w kolejnej podsekcji):
  ...
  * Po zakończeniu przebiegu należy przejrzeć logi (w szczególności log odbiorczy) i zapisać interesujące nas wyniki w celu późniejszego ich uśrednienia. Wyświetlenie logu w czytelnej formie w oknie terminala uzyskujemy komendą `/usr/bin/ITGDec <log-filename>`, gdzie `<log-filename>` to nazwa pliku z logiem podana w komendzie startującej generator `ITGSend`.

  W przypadku konieczności restartowania odbiornika czy wystąpienia problemów ze środowiskiem całą sieć można usunąć (w celu ponownego utworzenia) skryptem clean.sh: `sudo ./clean.sh` (komunikaty o błędach dotyczące nieistniejących urządzeń należy zignorować).

  ## Parametryzacja strumienia ruchu (w ramach elementarnego eksperymentu)

  ### Podstawowe strumienie ruchu i przykłady

Jak już wspomniano, wykorzystujemy generator ruchu pakietowego D-ITG. Ma on dość dobry [manual (format pdf)](https://traffic.comics.unina.it/software/ITG/manual/D-ITG-2.8.1-manual.pdf) i tutaj pomijamy szczegółowy opis zasad jego wykorzystania; interesujące nas definicje/opisy zamieszczone są tam na stronie 13 (_Inter-departure time options_); końcowa część dokumentu zawiera przykładowe komendy dla różnych przypadków strumieni. Dodatkowo, w naszym skrypcie `lbr.sh` (linie 209-227) znajdują się skomentowane przykłady ("działających") komend przeznaczonych do otwierania strumieni ruchu na nasze potrzeby. Na nich powinniśmy bazować, dostosowując ich parametry do konkretnych potrzeb poszczególnych eksperymentów.

Dla typowych opisów strumieni pakietów obowiązuje też uwaga interpretacyjna dotycząca odstępu czasu między pakietami zamieszczona na stronie **14** manuala:
...
Tymczasem w przypadku źródeł typu ON/OFF (patrz poniżej) interpretacja parametrów zmiennych losowych określanych w ramach opcji `-B` (czasy trwania stanów ON i OFF) musi być zmieniona na _czas-w-milisekundach_ (np. wyrażenie `-B C 100 C 500` będzie definiować strumień o stałym czasie trwania stanu ON równym 100 ms i stałym czasie trwania stanu OFF równym 500 ms, a nie 100 lub 500 pakietów/sek; charakter napływu pakietów w stanie ON będzie wtedy opisany odrębnym parametrem umieszczonym w głównej części komendy (czyli przed sekcją `-B`), np. `-C 1000` oznaczałoby wtedy stałą szybkość gnerowania w stanie ON równą 1000 pakietów/sek).
...
`-B <opis-czasu-ON> <opis-czasu-OFF>` - źródło ma być typu ON/OFF z opisem czasu trwania stanów ON/OFF podanym parametrami `<opis-czasu-ON>` i `<opis-czasu-OFF>`. Jak już wcześniej wspomniano, każdy ze składników `<opis-czasu-ON>` i `<opis-czasu-OFF>` można opisać stosując składnię ze strony 13 manuala, zmieniając tylko interpretację _czasu pomiędzy kolejnymi pakietami_ na, odpowiednio, _czas trwania okresów ON_ i _czas trwania okresów OFF_. **WAŻNE: blok `-B` - jeśli występuje - musi znajdować się na samym końcu komendy**; jeśli blok `B` nie występuje, wtedy cała konenda stosuje sie tak, jakby czas OFF był równy zero (źródło cały czas pozostaje w stanie ON);
...
`/usr/bin/ITGSend -a 10.0.0.2 -T UDP -c 1200 -C 200 -t 15000` - uruchamiana aplikacja (nasz generator ITGSend) ma słać ruch na adres 10.0.0.2, stosowć protokół UDP (-T UDP), formować pakiety o stałej długości 1500 bajtów (-c 1200) z szybkością generowania (tutaj w okresach ON, bo na końcu komendy jest blok `-B`) równą 200 pakietów/sek (-C 200), czas trwania przebiegu ma wynosić 15000ms (czyli 15 sekund; -t 15000). Zapis `-C 200` odpowiada konkretnej charakterystyce strumienia w stanie ON (rozkład jednopunktowy) - w ogólnym przypadku może tu jednak wystąpić dowolny z rozkładów/zapisów określonych na stronie **13** manuala D-ITG (chociaż w naszym laboratorium posługujemy się tylko rozkładem jednopunktowym);

`-j 1` - dodatkowo, żądamy, aby nadajnik starał wysyłać wszystkie teoretycznie przynależne strumieniowi pakiety, aby utrzymać wynikającą z teorii przepływność. Może być najsłabsza strona generatora, a trochę więcej na ten temat wyjaśniono w manualu na stronie **15** manuala. Ostatecznie, opisywana tu cecha może być przyczyną obserwowanej w naszych eksperymentach nie tylko niezgodności między oczekiwaną liczbą wygenerowanych pakietów a liczbą pakietów wygenerowanych faktycznie (np. dla rozkładu _constant_ (-C)), ale także zmiennej liczby faktycznie wygenerowanych pakietów w kolejnych przebiegach dla danego strumienia ruchu. Rozbiezność ta rośnie wraz z założoną szybkością generacji pakietów. W efekcie **wszelkie analizy i wnioski wystarczy opierać na danych z logów strony odbiornika `ITGRecv`**, logów nadajnika w ogóle można nie generować, a wyniki dla danego strumienia należy uśredniać z wielu (np. 10) przebiegów. **Rezygnacja z logów nadajnika jest tym bardziej korzystna, że dodatkowo oszczędza czas procesora i nieco poprawia ogólną jakość wyników.**
